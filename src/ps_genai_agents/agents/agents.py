...
def create_simple_text2cypher_agent(
    ...
    generate_cypher = create_text2cypher_generation_node(llm=llm, graph=graph)
...
def create_text2cypher_with_visualization_agent(
    llm: BaseChatModel, graph: Neo4jGraph, scope_description: Optional[str] = None
) -> CompiledStateGraph:
    """
    ...
    text2cypher = create_simple_text2cypher_agent(llm=llm, graph=graph)
...
def viz_mapper_edge(state: OverallState) -> List[Send]:
    ...
    for idx in indexes:
        cypher_state: CypherState = state.get("cyphers")[idx]
        task = Send(
            "visualize",
            {
