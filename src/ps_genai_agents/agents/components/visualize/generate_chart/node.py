...
import pandas as pd
from agents.components.state import OverallState, VisualizationState
from agents.components.visualize.generate_chart.charts import (
    create_bar_plot,
    create_line_plot,
    create_scatter_plot,
)
...
def create_chart_generation_node() -> Callable[[VisualizationState], OverallState]:
    """
    ...
    def generate_chart(state: VisualizationState) -> OverallState:
        """
        ...
        steps = state.get("steps") + ["generate_chart"]
        return {
            "visualizations": [
                {
